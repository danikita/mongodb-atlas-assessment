...
APP_PROJECT_MAP = {
    "5c893a855538555c636b3621": "5c893a855538555c636b3621",
    "69efbf887b99091ecf86face": "69efbf630bf262a89b8aa0c5"
}
    ...
    if resp.status_code == 200:
        ...
        for c in clusters:
            ...
            assessment["infrastructure"]["clusters"].append({
                ...
                "customer_managed_keys": c.get("encryptionAtRestProvider") is not None,
...
print(f"{GREEN}OK{RESET} - Arquivo JSON gerado:")
...
try:
    ...
    for p in final_output["projects"]:

        project_name = p.get("project_name")
        project_id = p.get("project_id")

        clusters = p.get("infrastructure", {}).get("clusters", [])
        apps = p.get("app_services", {}).get("apps", [])
        ...
        for app in apps:
            for t in app.get("triggers", []):
                ...
                if t.get("function_analysis", {}).get("uses_http"):
                    uses_http = True

                if t.get("function_analysis", {}).get("uses_collection"):
        ...
        summary_rows.append({
            ...
            "data_federation_enabled": p.get("infrastructure", {}).get("data_federation_enabled"),
            "private_endpoints_enabled": p.get("infrastructure", {}).get("private_endpoints_enabled")
        })

    with pd.ExcelWriter(excel_file) as writer:

        pd.DataFrame(summary_rows).to_excel(writer, "Summary", index=False)
        pd.DataFrame(cluster_rows).to_excel(writer, "Clusters", index=False)
        pd.DataFrame(trigger_rows).to_excel(writer, "Triggers", index=False)

    print(f"{GREEN}OK{RESET} - Excel gerado:")
    print(os.path.abspath(excel_file))

except Exception as e:
    print(f"{RED}ERRO{RESET} ao gerar Excel")
    print(e)
